...
full_unit_names = 1  # determines whether full unit names will be displayed (1), or abbreviations (0)

# Defines the Frames
leftFrame = Frame(root, bg="royal blue")
rightFrame = Frame(root, bg="gainsboro")
bottomFrame = Frame(root, bg="gainsboro")
veryBottomFrame = Frame(root, bg="gainsboro")
...
# Label - left
label_left = Label(leftFrame, text="From:", bg="royal blue", fg='white')
...
# Entry Box - left
entry_box_1 = Entry(leftFrame, width=34, bg="white", highlightbackground="light blue")
...
# Label - right
#label_right = Label(rightFrame, text="To:", bg="gainsboro", fg="black")
#label_right.grid(row=0, column=1, padx=10, pady=5, sticky=W)

# Option Menu - right
outputUnit = StringVar(rightFrame)
outputUnit.set(options_list[0])
unit_selector2 = OptionMenu(rightFrame, outputUnit, *options_list)
unit_selector2.grid(row=0, column=2, sticky=W, padx=10, pady=5)
...
# Entry Box - right
entry_box_2 = Entry(rightFrame, width=37, bg="gainsboro", highlightbackground='#FAFAFA', fg='black', relief=FLAT)
entry_box_2.grid(row=1, column=0, padx=10, pady=10, sticky=W, columnspan=3)
...
swap_units.grid(row=0, column=0, padx=10, pady=5, sticky=W)
...
def generate_answer():
    ...
    if insert_commas == 1:
        if full_unit_names == 1:
            entry_box_2.insert(0, f'{rounded:,} {long_unit}')
        elif full_unit_names == 0:
            entry_box_2.insert(0, f'{rounded:,} {short_unit}')
...
scale_label = Label(veryBottomFrame, text="Number of decimal places to round to:",
                    width=35,
                    bg='gainsboro',
...
full_names_checkbox = Checkbutton(veryBottomFrame, text="Use abbreviated units in result",
                             relief=FLAT,
                             bg='gainsboro',
                             command=abbreviated_units)
full_names_checkbox.grid(row=2, column=0, pady=10)
